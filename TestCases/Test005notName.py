...
# assert "Please fill all information in Thai or English only. / กรุณากรอกข้อมูลทั้งหมดเป็นภาษาไทย หรือภาษาอังกฤษเท่านั้น"  in driver.page_source
...
class testnotName(unittest.TestCase):

    def setUp(self):
        self.driver = webdriver.Firefox()

    def testnotName(self):
        driver = self.driver
        driver.get("https://www.gmm-tv.com/shop/index.php?route=account/register")
        driver.set_window_size(1358, 782)
        # driver.find_element(By.ID,'input-firstname').send_keys('ณาตหชา')
        ...
        driver.find_element(By.XPATH,"//input[@name='agree']").click()
        driver.find_element(By.XPATH,"//input[@value='Submit / ลงทะเบียน']").click()
        self.assertIn("Please fill all information in Thai or English only. / กรุณากรอกข้อมูลทั้งหมดเป็นภาษาไทย หรือภาษาอังกฤษเท่านั้น", driver.page_source)
        driver.close()
